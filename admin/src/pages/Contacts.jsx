      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="font-montserrat-regular-400 text-black-light">Loading contacts...</p>
          </div>
        </div>
      </div>
    );
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px]">
              <thead className="bg-primary-light border-b border-gray-200">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-montserrat-semibold-600 text-black whitespace-nowrap">Contact</th>
                  <th className="px-6 py-4 text-left text-sm font-montserrat-semibold-600 text-black whitespace-nowrap">Service</th>
                  <th className="px-6 py-4 text-left text-sm font-montserrat-semibold-600 text-black whitespace-nowrap">Message</th>
                  <th className="px-6 py-4 text-left text-sm font-montserrat-semibold-600 text-black whitespace-nowrap">Date</th>
                  <th className="px-6 py-4 text-center text-sm font-montserrat-semibold-600 text-black whitespace-nowrap">Actions</th>
                </tr>
              </thead>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex flex-col space-y-1 mt-1">
                            <a href={`mailto:${contact.email}`} className="text-xs text-primary hover:underline flex items-center space-x-1">
                              <Mail className="w-3 h-3" />
                              <span>{contact.email || 'N/A'}</span>
                            </a>
                            {contact.phone && (
                              <a href={`tel:${contact.phone}`} className="text-xs text-black-light hover:underline flex items-center space-x-1">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-montserrat-medium-500 text-black-light mb-1">Name</label>
                  <div className="flex items-center space-x-2 text-black font-montserrat-medium-500">
                    <User className="w-4 h-4" />
                    <span>{currentContact?.name || selectedContact?.name || 'N/A'}</span>
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-montserrat-medium-500 text-black-light mb-1">Email</label>
                  <a href={`mailto:${currentContact?.email || selectedContact?.email}`} className="flex items-center space-x-2 text-primary hover:underline">
                    <Mail className="w-4 h-4" />
                    <span>{currentContact?.email || selectedContact?.email || 'N/A'}</span>
                  </a>
                </div>

                {currentContact?.phone || selectedContact?.phone ? (
                  <div>
                    <label className="block text-sm font-montserrat-medium-500 text-black-light mb-1">Phone</label>
                    <a href={`tel:${currentContact?.phone || selectedContact?.phone}`} className="flex items-center space-x-2 text-black hover:underline">
                      <Phone className="w-4 h-4" />
                      <span>{currentContact?.phone || selectedContact?.phone}</span>
                    </a>
                  </div>
                ) : null}

                <div>
                  <label className="block text-sm font-montserrat-medium-500 text-black-light mb-1">Service</label>
                  <span className="inline-flex px-3 py-1 bg-blue-100 text-blue-800 text-xs font-montserrat-medium-500 rounded-full">
                    {getServiceLabel(currentContact?.service || selectedContact?.service || 'general')}
                  </span>
                </div>

                <div className="md:col-span-2">
                  <label className="block text-sm font-montserrat-medium-500 text-black-light mb-1">Date</label>
                  <div className="flex items-center space-x-2 text-black">
                    <Calendar className="w-4 h-4 text-black-light" />
                    <span>{formatDate(currentContact?.createdAt || selectedContact?.createdAt)}</span>
                  </div>
                </div>

                <div className="md:col-span-2">
                  <label className="block text-sm font-montserrat-medium-500 text-black-light mb-2">Message</label>
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <p className="text-sm font-montserrat-regular-400 text-black whitespace-pre-wrap">
                      {currentContact?.message || selectedContact?.message || 'N/A'}
                    </p>
                  </div>
                </div>
              </div>
